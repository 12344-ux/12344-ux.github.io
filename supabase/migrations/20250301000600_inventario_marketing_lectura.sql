...
--
-- PROBLEMA QUE RESUELVE: las tablas del modulo Inventario tienen RLS
-- tiene_modulo('inventario') (ver 20250301000300_inventario_rls.sql). Un
-- usuario SOLO de marketing quedaria bloqueado al leer el libro. Este archivo
-- ANADE una politica SELECT extra que ADEMAS permite leer movimientos_inventario
-- y productos cuando tiene_modulo('marketing'). Es la opcion mas simple y con
-- mejor UX: no obliga a mezclar permisos de dos areas para una tarea de solo
-- lectura, y mantiene el candado real en los datos.
--
...
--
-- IDEMPOTENTE: drop policy if exists + create policy, y las tablas ya tienen
-- RLS habilitada por 20250301000300 (se re-asegura por si se corre suelto).
...
-- ------------------------------------------------------------
-- productos: SELECT adicional para el modulo 'marketing'.
-- Convive con productos_select_modulo (tiene_modulo('inventario')): en RLS,
-- basta con que UNA politica permisiva de SELECT sea verdadera.
-- ------------------------------------------------------------
