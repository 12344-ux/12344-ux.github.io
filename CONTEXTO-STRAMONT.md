...
> **Última actualización:** 28 de julio de 2026 (sesión: **AUDITORÍA DEL FLUJO DE PAGO WOMPI** — el primer encargo explícito del dueño; se cazaron 2 bugs silenciosos y se cerró el invariante "nada sin pago aprobado"). 👉 **El detalle completo de ESTA sesión está en la nueva sección 2I — léela primero.** (La 2H es la sesión de Wompi en producción; la 2G, rediseño de landing + guía.)
...
### PENDIENTE que sigue abierto (decisión del dueño, NO se tocó)
- **Limpieza de apuntes de carritos abandonados / rechazados / reversados** (era 2H → Pendientes #3). Los apuntes se suben **antes** de pagar (única vía técnica), así que un checkout abandonado —o cada reintento de pago, que genera un `pedido_id` nuevo y **re-sube** los mismos archivos— deja apuntes huérfanos en Storage y **nada los borra hoy**. Opciones ofrecidas: (a) limpieza **automática** diaria (cron/programador en Supabase) o (b) **botón manual** "🧹 limpiar intentos sin pagar" en el tablero. **El dueño aún no elige** → no se implementó (se hará cuando decida). *(Chispa crítica: con volumen, esto puede llenar el 1 GB de Storage con basura de intentos fallidos; conviene resolverlo antes de escalar campañas.)*
