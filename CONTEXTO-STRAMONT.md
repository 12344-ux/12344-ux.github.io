...
> **EN CURSO — Frente C (rediseño de la landing):** la sección **"El Cambio"** quedó rehecha al **layout zig-zag** (info + imagen grande alternadas, captura REAL de la guía, ✕ en rojo) → está en el **PR #91, PENDIENTE DE MERGE** (en `main` sigue la versión apretada anterior). **El dueño quiere seguir puliendo "El Cambio".** El **hero (C2) está pendiente**. 👉 **Para retomar exactamente ahí, lee la sección 2F.**
>
...
- **Frente C1 — sección "El Cambio" (index.html): REDISEÑADA, EN PULIDO (aquí es donde seguimos).** Comparación Sin/Con Stramont fiel al mockup del "Kiro de ideas", con las imágenes VISIBLES. **Historia (para entender el estado):** primero quedó en **2 tarjetas apretadas** (se mergeó en el #90) → el dueño observó que la **captura de la guía se veía diminuta, oscura y cansaba la vista** → se rehízo al **layout ZIG-ZAG a ancho completo**: *fila 1 Sin Stramont* (info izq + 3 fotos de apuntes en abanico der), *fila 2 Con Stramont* (**captura de la guía GRANDE** izq + beneficios der), stats a lo ancho debajo + CTA. Las **✕ de "Sin Stramont" en rojo coherente** (`#E24C4C`, badge rojo suave). Íconos ✓/✕ y la flecha del enlace como **SVG inline** (no dependen de fuente). CSS autocontenido en un `<style>` de `index.html` (NO se toca `estilos.css` → sin cache-bust; clases `cambio-wrap`, `c2-row`, `c2-row.alt`, `c2-info`, `c2-media`, `c2-guia`, `c2-notas`, `c2-stats`). Números REALES: 6 secciones · 14 conceptos · 7 ejemplos · 24 preguntas. "Mejores resultados" → "más confianza para tu examen: llegas listo, no a ciegas".
  - **⚠️ ESTADO EXACTO PARA RETOMAR:** el zig-zag está en el **PR #91 (PENDIENTE DE MERGE)**. En `main` todavía está la versión apretada (marcador `class="cambio2"`); el zig-zag usa `class="cambio-wrap"`. Si el PR #91 ya se mergeó cuando leas esto, `main` tendrá `cambio-wrap`. **El dueño dijo que aún quiere hacer cambios / seguir puliendo "El Cambio"** → ese es el siguiente foco: pídele qué quiere ajustar y itera (mismo estilo: rama+PR, verificando con Chromium en móvil y PC).
  - **Assets en el repo:** `apunte-seg-1/2/3.jpg` (fotos de apuntes reales de Segmentación) y `guia-segmentacion-nueva.jpg` (captura del tablero, la que va en "Con Stramont").
