...
> 10. **🌎 PÁGINA "POR QUÉ WOMPI" + PAGOS INTERNACIONALES (§2N):** nueva `por-que-wompi.html` (ethos de pago apoyado en Bancolombia/Grupo Cibest, con datos verificables) + explicación del FX + FAQ, enlazada con **link azul discreto desde el checkout**. Wording honesto ("aceptamos tarjetas internacionales", no "garantizado en todo el mundo") porque el dueño no puede probar un pago del exterior; sin riesgo de plata porque el cobro se confirma por webhook. Solo frontend.
>
...
## 2N. Página "Por qué Wompi" + pagos internacionales (ethos de pago) — 28-jul

**El encargo (discutido antes de construir):** el dueño cayó en cuenta de que, gracias a Wompi, **puede vender internacionalmente** (tarjetas de otros países), y quería (a) resaltar que Stramont ya funciona fuera de Colombia y (b) una sección "¿Por qué Wompi?" que construya **ethos/credibilidad** apoyándose en la marca Wompi/Bancolombia. Lo **auditamos primero**.

**Decisiones clave de la discusión:**
- **Wording honesto, no garantía absoluta.** El dueño confirmó que su panel de Wompi ya muestra las opciones de tarjeta, **pero no puede probar un pago desde otro país** (no tiene familia afuera). Por eso NO afirmamos "funcionamos garantizado en todo el mundo"; afirmamos **capacidad verificable**: "aceptamos tarjetas internacionales Visa/Mastercard". Red de seguridad: como el cobro solo se confirma por **webhook**, si una tarjeta del exterior se rechaza **no se cobra nada** → el "asumir que funciona" no arriesga plata, solo una venta puntual. (Si se quiere certeza: tarjeta fintech tipo Wise/Payoneer.)
- **NO en la landing.** Por decisión del dueño, va como **página aparte** (como el enlace de Términos), enlazada con un **link azulito discreto desde el checkout**. No satura el paso de pago.
- **FX: divulgar, no complicar.** Nada de multimoneda ni de absorber comisiones (producto de $3–$5). Se **explica claro**: precio en USD, cobro en COP a la tasa del día; el banco del comprador convierte a su moneda y **puede** añadir una comisión internacional **ajena a Stramont**.
- **Solo datos verificables** (nada inventado). Se confirmó por fuentes: Wompi es la **plataforma de pagos del Grupo Cibest** (el mismo grupo de Bancolombia); cifras 2025 (**+13 M de personas, +40.000 negocios activos/mes, ~160 M de transacciones**). **NO** se afirmó certificación **PCI DSS** (no se pudo verificar de forma directa) → se usó "estándares de seguridad de la industria + tokenización + respaldo de Bancolombia".

**Qué se construyó (frontend, solo GitHub Pages):**
- **`por-que-wompi.html` (NUEVA)** — página compuesta y premium, molde de `quienes-somos.html`: **reusa el shell de `estilos.css`** + CSS propio con prefijo **`pw-`**, **NO toca `estilos.css`**. Secciones: hero + lockup Wompi/Bancolombia · banda de **cifras** (con "Fuentes" citadas y enlazadas) · **"¿Por qué elegimos Wompi?"** (4 tarjetas: respaldo Bancolombia · seguridad nivel bancario · comercio verificado · Stramont nunca ve tu tarjeta) · **"Paga desde cualquier país"** + checks · **caja FX** "¿En qué moneda te cobran?" · **FAQ** (5) · CTA "Volver a mi compra" · fuentes + **descargo de marcas** (Wompi/Bancolombia son de sus titulares; Stramont es un comercio que usa Wompi, sin implicar alianza). Iconografía Lucide inline, sin emojis.
- **`checkout.html` (editado)** — dentro del bloque "Pago seguro (Wompi)" se añadió `.ps-intl`: línea "También aceptamos **tarjetas internacionales**" + **enlace azul discreto** a `por-que-wompi.html` (patrón idéntico al de Términos). No toca `estilos.css` (el CSS va en el `<style>` propio del checkout).

**Verificado con Playwright** (escritorio 1360 + móvil 390): 0 errores de consola, sin overflow horizontal, CTA y back-link → `checkout.html`, logo de Wompi carga. **Deploy:** solo frontend (GitHub Pages), recarga forzada.
...
| `por-que-wompi.html` | **NUEVA (§2N).** Página de **ethos de pago**: por qué Wompi (respaldo Bancolombia/Grupo Cibest, seguridad, comercio verificado, no guardamos tu tarjeta) + **pagos internacionales** + explicación del **FX** (USD mostrado / COP cobrado / conversión del banco del comprador) + FAQ. Molde de `quienes-somos.html`: reusa el shell de `estilos.css` + CSS propio **prefijo `pw-`**, **NO toca `estilos.css`**. Enlazada con **link azul discreto desde `checkout.html`** (bloque "Pago seguro"). Solo datos verificables (con fuentes citadas); NO afirma PCI DSS. |
...
7. **Rutina fija tras CUALQUIER merge (no esperes a que el dueño reporte algo raro — verifícalo tú de una vez):**
   ...
   3. **Si está `queued`, dale primero 1-2 minutos y vuelve a consultar antes de reaccionar** — muchas veces termina solo y pasa a `success` (ya ha pasado: se creó una rama de "reintento" de más porque no se le dio tiempo). Un atasco *real* es cuando sigue en `queued` varios minutos después, o si es `failure`/`cancelled` (esto último puede pasar si se mergean dos PRs casi juntos, normal, no es lo mismo que un atasco).
   4. **No verifiques el sitio en vivo con `curl`/`fetch` para decidir si el deploy funcionó** — el CDN de GitHub Pages cachea (`cache-control: max-age=600`), así que puedes ver contenido viejo aunque el deploy YA haya terminado bien. La fuente de verdad es el estado del run en Actions, no lo que responde el dominio.
   5. Si de verdad quedó atascado/fallido tras esperar: sincroniza `main` (`pull_repository`), **crea una rama nueva** (nunca reutilices una ya mergeada — ver regla 2 de esta lista) con un commit trivial (ej. cambiar el comentario `rebuild-trigger` al inicio de `simulacion.html`) para forzar un run limpio, y ábrelo en un PR nuevo. NO uses "Re-run", se puede quedar atascado igual.
   6. Solo después de confirmar el run en verde (en Actions, no por curl), dile al dueño que ya puede hacer la recarga forzada (Ctrl/Cmd+Shift+R) — el navegador del usuario no tiene el mismo caché de 10 min del CDN, así que para él sí refleja lo nuevo de inmediato tras el deploy.
