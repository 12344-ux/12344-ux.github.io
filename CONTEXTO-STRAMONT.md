...
> **Última actualización:** 24 de julio de 2026 (sesión: **WOMPI EN PRODUCCIÓN** — pago real con confirmación blindada por webhook). 👉 **El detalle completo de ESTA sesión está en la nueva sección 2H — léela primero.** (La 2G es la sesión anterior: rediseño de landing + guía.)
...
1. **REDESPLEGAR la función `informe` en Supabase** (lo más urgente): los fixes #125 y #126 viven en `supabase/functions/informe/index.ts`; el merge a `main` NO despliega funciones. Hasta redesplegarla, el tablero sigue contando mal. Copiar el código de `main` y pegar en Edge Functions → informe → Deploy (JWT OFF). *(El dueño quedó en este paso justo al cerrar; confirmar que lo hizo.)*
2. **Limpieza de apuntes de carritos abandonados** (DECISIÓN PENDIENTE del dueño): los apuntes se suben ANTES de pagar (única forma técnica), así que un checkout abandonado deja apuntes en Storage y nada los borra hoy. Se ofreció: (a) limpieza **automática** diaria (requiere cron/programador en Supabase) o (b) **botón manual** "🧹 limpiar intentos sin pagar" en el tablero. El dueño aún no eligió — preguntar y construir.
3. **Label USD vs COP** (opcional): el sitio muestra USD y cobra COP con nota de transparencia; si el dueño quiere, alinear textos. Por ahora se deja así (decisión tomada: mostrar USD).
4. **Vigilar los primeros pagos reales.** El pago de prueba real (tarjeta del hermano, $3) funcionó punta a punta. Si un pago no se marca "aprobado", revisar que el webhook recibió el evento (logs de la función en Supabase) y que la URL de eventos + `WOMPI_EVENTS_SECRET` están bien.
