...
### Decisiones ABIERTAS de esta tanda (en conversación con el dueño, aún sin implementar)
- **Kit por correo:** revisado. Los 5 archivos del `kit-stramont.zip` están OK y el enlace carga en vivo (200). La plantilla **Notion** es un `.md` limpio de 697 B que se importa a Notion → **recomendación: MANTENERLA** (Notion es popular entre estudiantes, pesa nada). Sin cambios salvo que el dueño decida quitarla.
- **Correo 3 (opiniones) + idea de IG en testimonios:** el dueño quiere que, si el cliente autoriza mostrar su testimonio en la landing, pueda **dejar su IG** (venta: la visibilidad de la página puede subir la visibilidad de su perfil). **Pendiente de evaluar y aprobar** antes de construir (implicaría columna nueva en `pedido_feedback`, campo en `feedback.html`, ajuste de la función `feedback` y del Correo 3). 
- **Renombrar `simulacion.html`:** la URL de pago dice "simulacion" y en producción resta confianza. Pendiente decidir el nombre nuevo (ej. `comprar.html`/`pagar.html`) y dejar `simulacion.html` como **redirect** para no romper enlaces ya compartidos.
