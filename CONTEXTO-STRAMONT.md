...
- **PENDIENTE (el resto de "toda la marca"):** aplicar el mismo lenguaje a los **correos** (`correo-confirmacion`, `correo-entrega`, `correo-feedback`), **`checkout.html`**, `quienes-somos`, `condiciones` y `privacidad`. La landing es solo el primer frente. (Deploy de la landing: solo GitHub Pages.)
